...
class Aliyun extends Server
{
    ...
    public function setAcl($fileName, $acl)
    {
        try {
            $ossClient = new OssClient(
                $this->config['access_key_id'],
            ...
            $ossClient->putObjectAcl($this->config['bucket'], $fileName, $acl);
        } catch (OssException $e) {
            $this->error = $e->getMessage();
            return false;
        }
    ...
    public function getSignUrl($fileName)
    {
        try {
            $ossClient = new OssClient(
                $this->config['access_key_id'],
            ...
            $signedUrl = $ossClient->signUrl($this->config['bucket'], $fileName, 3600);
            return $signedUrl;
        } catch (OssException $e) {
            $this->error = $e->getMessage();
            return false;
